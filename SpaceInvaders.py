...
# loading images
RED_SPACESHIP = pygame.image.load(os.path.join("pixel_ship_red_small.png"))
GREEN_SPACESHIP = pygame.image.load(os.path.join("pixel_ship_green_small.png"))
BLUE_SPACESHIP = pygame.image.load(os.path.join("pixel_ship_blue_small.png"))
# lasers
...
# Player ship
YELLOW_SPACESHIP = pygame.image.load(os.path.join("pixel_ship_yellow.png"))
# Background
    ...
    while run:
        ...
        if len(enemies) == 0:
            ...
            for i in range(wave_length):
                enemy = Enemy(random.randrange(50, WIDTH - 100), random.randrange(-1500*(level/4), -100),
        ...
        if keys[pygame.K_a] and player.x - player_vel > 0 or keys[pygame.K_LEFT] and player.x - player_vel > 0:  # left
            player.x -= player_vel
        if keys[pygame.K_d] and player.x + player_vel + player.get_width() < WIDTH or keys[
        ...
        if keys[pygame.K_w] and player.y - player_vel > 0 or keys[pygame.K_UP] and player.y - player_vel > 0:  # up
            player.y -= player_vel
        if keys[pygame.K_s] and player.y + player_vel + player.get_height() + 15 < HEIGHT or keys[
